...
- `packages/container/` - Main DI container library (`@stackra-inc/react-di`)
...
│   └── container/                # @stackra-inc/react-di package
│       ├── src/                  # Source code
│       ├── __tests__/            # Tests
│       ├── dist/                 # Build output
...
```json
{
  "name": "@stackra-inc/react-di-monorepo",
  "version": "1.0.4",
  "private": true,
...
- Use `@stackra-inc/` scope for all packages
...
export default defineConfig({
  ...
  target: 'es2020',
  platform: 'neutral',
  external: ['@stackra-inc/react-di', 'react'],
  splitting: false,
...
pnpm --filter @stackra-inc/react-di build

# Run dev mode for specific package
pnpm --filter @stackra-inc/react-di dev
...
pnpm --filter @stackra-inc/react-di add inversify
 ...
 * from @stackra-inc/support for consistent registry API.
 *
 * Key Features:
 ...
 *
 * @module @stackra-inc/{package-name}
 * @category Registries
 */

import { BaseRegistry } from '@stackra-inc/support';
import { Injectable } from '@stackra-inc/react-di';
 ...
 * @example
 * ```typescript
 * import { useInject } from '@stackra-inc/react-di';
 * import { ComponentRegistryService } from '@stackra-inc/{package-name}';
 *
 ...
 * Basic usage:
 * ```typescript
 * import { Service, useHook } from '@stackra-inc/{package-name}';
 *
 * // Usage example
 * ```
 *
...
import { Injectable, Inject } from '@stackra-inc/react-di';
 ...
 * @example Basic Usage
 * ```typescript
 * import { PackageModule } from '@stackra-inc/{package-name}';
 *
 * @Module({
...
\`\`\`bash pnpm add @stackra-inc/{package-name} \`\`\`
...
\`\`\`typescript import { Service } from '@stackra-inc/{package-name}';
...
3. **Use Dependency Injection** - Leverage `@stackra-inc/react-di` for loose
   coupling
