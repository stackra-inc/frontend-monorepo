 ...
 *
 * Demonstrates @stackra-inc/ts-config:
...
import { ConfigService } from '@stackra-inc/ts-config';
    ...
    <section className="flex flex-col gap-8 py-8 md:py-10">
      ...
      <div>
        ...
        <p className={subtitle({ class: 'mt-2' })}>
          @stackra-inc/ts-config — environment-aware configuration with typed access
        </p>
      </div>
        ...
        <Card.Content>
          <div className="flex flex-col gap-3">
            {entries.map((entry) => (
              <div
                key={entry.key}
                className="border-divider flex items-center justify-between rounded-lg border px-4 py-3"
              >
                ...
                <span className="text-foreground font-mono text-sm">{String(entry.value)}</span>
              </div>
            ))}
          </div>
        </Card.Content>
      ...
      <Card>
        <Card.Header className="flex flex-col items-start gap-1">
          ...
          <p className="text-default-500 text-sm">Boolean config values used as feature toggles.</p>
        </Card.Header>
        <Separator />
        <Card.Content>
      ...
      <Card>
        ...
        <Separator />
        <Card.Content>
          <pre className="bg-default-100 text-foreground overflow-x-auto rounded-lg p-4 font-mono text-xs">
            {`// In your AppModule:
