...
/** Color mapping for log level chips. */
...
/**
 * LoggerPage — interactive showcase of the Logger package.
 *
 * Uses the useLogger() hook to access the DI-provided LoggerService
 * and renders a live log feed as the user triggers different log levels.
 */
export default function LoggerPage() {
  ...
  /** Append a log entry to the UI feed. */
    ...
    <section className="flex flex-col gap-8 py-8 md:py-10">
          ...
          <div className="flex flex-wrap gap-3">
            {(["debug", "info", "warn", "error", "fatal"] as const).map(
              (level) => (
                ...
                >
                  <Chip size="sm" color={LEVEL_COLORS[level]} variant="soft">
                    {level}
                  </Chip>
      ...
      <Card>
        <Card.Header className="flex flex-col items-start gap-1">
          ...
          <p className="text-sm text-default-500">
            Click to emit logs — they appear in the feed below and in the
            browser console.
          </p>
        </Card.Header>
        <Separator />
      ...
      <Card>
        <Card.Header className="flex items-center justify-between">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-default-500">
              {logs.length} entries (last 50 kept)
            </p>
          </div>
