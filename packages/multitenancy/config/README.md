...
`@stackra-inc/multitenancy` package.
...
```bash
cp node_modules/@stackra-inc/multitenancy/config/multitenancy.config.example.ts src/config/multitenancy.config.ts
```
...
import { defineConfig, subdomainPreset } from '@stackra-inc/multitenancy';
