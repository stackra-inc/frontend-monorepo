 ...
 * subscribes to the specified channel and event on mount, and automatically
 * cleans up on unmount. Re-subscribes when `channelName` or `eventName`
 * change.
 *
...
export function useChannel<T>(
  ...
  useEffect(() => {
    if (!enabled || !manager.isConnected()) return;

    let wrapper: ChannelWrapper;

    try {
      wrapper = manager.channel(channelName);
  ...
  }, [manager, channelName, eventName, enabled, connected]);
