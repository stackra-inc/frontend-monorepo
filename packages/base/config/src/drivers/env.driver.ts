...
import { Env, Str } from '@stackra/ts-support';
...
export class EnvDriver implements ConfigDriver {
  ...
  load(): Record<string, any> {
    ...
    // Try browser global first, then Env repository
    if (typeof window !== 'undefined' && (window as any)[globalName]) {
      ...
      this.config = { ...Env.getRepository() };
      this.log(`Loaded config from Env repository: ${Object.keys(this.config).length} keys`);
    }
